...
Component({
  ...
  methods: {
    ...
    _input(e) {
      ...
      let val = e.detail.value.replace(/[\D]/g, '');//过滤非数字字符
      if (inputsValue.indexOf(val) > -1&&val!=''){//过滤重复字符
        val = '';
        wx.showToast({
          title: '不存在重复数！',
          icon: 'loading',
      ...
      if (val.trim().length >= 1) {
        ...
        inputsValue[ind] = val.substr(-1, 1)
      } else {
        this._changeClass(ind - 1)
        inputsValue[ind] = ''
      }
      ...
      getApp().globalData.tempArr = this.data.inputsValue
